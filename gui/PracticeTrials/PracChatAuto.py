...
from ReadInput import practiceMultiAuto
from DataLogging.LogMulti import LogMulti, MultiCSV
from SingleTaskSummaries.SumChatAuto import SumChatAuto
import random
import datetime
...
class PracChatAuto(QMainWindow):
    def __init__(self):
        super().__init__()
        ...
        title = Title("Task: Practice Chat Box with Automation")
        ...
        storm_items = [StormItem(1, practiceMultiAuto.nav_storm_x[0], practiceMultiAuto.nav_storm_y[0]),
                      StormItem(2, practiceMultiAuto.nav_storm_x[1], practiceMultiAuto.nav_storm_y[1]),
                      StormItem(3, practiceMultiAuto.nav_storm_x[2], practiceMultiAuto.nav_storm_y[2]),
                      StormItem(4, practiceMultiAuto.nav_storm_x[3], practiceMultiAuto.nav_storm_y[3])]

        for item in storm_items:
            left_nav.addItem(item.pixmap_item)

        goal_items = [GoalItem(1, practiceMultiAuto.nav_goal_x[0], practiceMultiAuto.nav_goal_y[0]),
                      GoalItem(2, practiceMultiAuto.nav_goal_x[1], practiceMultiAuto.nav_goal_y[1]),
                      GoalItem(3, practiceMultiAuto.nav_goal_x[2], practiceMultiAuto.nav_goal_y[2]),
                      GoalItem(4, practiceMultiAuto.nav_goal_x[3], practiceMultiAuto.nav_goal_y[3])]
        
        #(self, idx, color_hex, color_text, x_pos, y_pos, curr_goal, goals_list, fuel, speed, angle, on_click_callback):
        uav_blue = UAVItem(1, "#90D5FF", "BLUE", practiceMultiAuto.nav_uav_x[0], practiceMultiAuto.nav_uav_y[0],
                           goal_items[0], goal_items, storm_items, practiceMultiAuto.nav_uav_fuel[0], 
                           practiceMultiAuto.nav_uav_speed[0], practiceMultiAuto.nav_path_angle[0], self.HandleClick, left_nav)
        ...
        uav_red = UAVItem(2, "#FF7F7F", "RED", practiceMultiAuto.nav_uav_x[1], practiceMultiAuto.nav_uav_y[1],
                          goal_items[1], goal_items, storm_items, practiceMultiAuto.nav_uav_fuel[1], 
                           practiceMultiAuto.nav_uav_speed[1], practiceMultiAuto.nav_path_angle[1], self.HandleClick, left_nav)
        UAVs.append(uav_red)
        ...
        uav_green = UAVItem(3, "#88E788", "GREEN", practiceMultiAuto.nav_uav_x[2], practiceMultiAuto.nav_uav_y[2],
                           goal_items[2], goal_items, storm_items, practiceMultiAuto.nav_uav_fuel[2], 
                           practiceMultiAuto.nav_uav_speed[2], practiceMultiAuto.nav_path_angle[2], self.HandleClick, left_nav)
        ...
        uav_yellow = UAVItem(4, "#FFEE8c", "YELLOW", practiceMultiAuto.nav_uav_x[3], practiceMultiAuto.nav_uav_y[3], 
                           goal_items[3], goal_items, storm_items, practiceMultiAuto.nav_uav_fuel[3], 
                           practiceMultiAuto.nav_uav_speed[3], practiceMultiAuto.nav_path_angle[3], self.HandleClick, left_nav)
    ...
    def CreateAutomationButton(self, label, state_var_name):
        ...
        start_on = getattr(practiceMultiAuto, oas_name, False)
        clickable = getattr(practiceMultiAuto, clickable_name, True)
    ...
    def StartSummary(self):
        ...
        self.showSum = SumChatAuto(summary)
...
class GenerateLevel(QWidget):
    def __init__(self, color_hex: str, idx: int, color_text: str):
        ...
        top_bar.setPos(0, practiceMultiAuto.gauge_mean[idx] - practiceMultiAuto.gauge_dist[idx])
        top_bar.setPen(pen)

        low_bar = level.addRect(0, 0, 40, 0)
        low_bar.setPos(0, practiceMultiAuto.gauge_mean[idx] + practiceMultiAuto.gauge_dist[idx])
        low_bar.setPen(pen)
    ...
    def ResetLevel(self, idx:int):
        ...
        self.AnimateHeight(practiceMultiAuto.gauge_mean[idx])
        self.monitor_level = practiceMultiAuto.gauge_mean[idx]
        self.reset_button.setStyleSheet(self.default_btn)
        self.reset = True
    ...
    def TimerDelay(self):
        delay = random.randint(practiceMultiAuto.gauge_timer[0]*1000, practiceMultiAuto.gauge_timer[1]*1000)
        self.reset = False
        self.timer.start(delay)

    def ChangeHeight(self, idx: int):
        new_height = max(10, min(290, int(random.normalvariate(practiceMultiAuto.gauge_mean[idx], practiceMultiAuto.gauge_sd[idx]))))
...
class ChatWidget(QWidget):
    def __init__(self, context_items, chat_auto_btn, chat_auto_btn2):
        ...
        #delay = random.randint(practiceMultiAuto.chat_timer[0]*1000, practiceMultiAuto.chat_timer[1]*1000)
    ...
    def compute_answer(self):
        ...
        if "GAUGE" in question:
            ...
            if gauge:
                mean, dist = practiceMultiAuto.gauge_mean[idx], practiceMultiAuto.gauge_dist[idx]
    ...
    def add_random_message(self):
        ...
        probability2 = random.randint(practiceMultiAuto.chat_auto2[0], practiceMultiAuto.chat_auto2[1])
        ...
        delay = random.randint(practiceMultiAuto.chat_timer[0]*1000,
                           practiceMultiAuto.chat_timer[1]*1000)
        self.timer.start(delay)
