...
def read_single():
    try:
        ...
        with open('input_files/single_task_input.csv', 'r', newline='') as filename: 
            ...
            nav_goal_x = [int(item) for item in rows[7][1:] if item.strip()]
            nav_goal_y = [int(item) for item in rows[8][1:] if item.strip()]
