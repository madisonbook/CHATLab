...
from Instructions.InstrPracUAVAuto import InstrPracUAVAuto

class SumMtrAuto(QMainWindow):
    def __init__(self, stats: list[int]):
        ...
        title = Title("Monitor Levels with Automation Task Complete")
        main_layout.addWidget(title)
        main_layout.addSpacing(10)

        participate = Subtitle("Congratulations! You have successfully completed the Monitor Levels with Automation task.")
...
def StartUAV(self):
    self.instrUAV = InstrPracUAVAuto()
    self.instrUAV.show()
    self.close()
