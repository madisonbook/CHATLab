...
class Multi_Auto1(QMainWindow):
    def __init__(self):
        ...
        goal_items = [
            GoalItem(i + 1, multiauto1Input.nav_goal_x[i], multiauto1Input.nav_goal_y[i])
            for i in range(len(multiauto1Input.nav_goal_x))
        ]
    ...
    def UpdateButtons(self, uav: UAVItem):
        ...
        if nav_auto2 and self.path_auto_fires:
            if uav.hit_chancea < uav.hit_chanceb + multiauto1Input.nav_auto1_path:
    ...
    def ShouldAutomationFire(self, auto_type: str) -> bool:
        if auto_type == "warn":
            low, high = multiauto1Input.nav_auto1
        elif auto_type == "path":
            low, high = multiauto1Input.nav_auto2
        else:
            return False
    ...
    def CreateAutomationButton(self, label, state_var_name, on_enable=None):
        ...
        start_on = getattr(multiauto1Input, oas_name, False)
        clickable = getattr(multiauto1Input, clickable_name, True)
    ...
    def _OnAutomationEnabled(self, state_var_name):
        ...
        # --- Chat Auto 1: highlight chat box if there's an unanswered message ---
            ...
            if self.chat_box.latest_message.text() not in ("", "Waiting..."):
                probability = random.randint(multiauto1Input.chat_auto1[0], multiauto1Input.chat_auto1[1])
                rand_num = random.randint(0, 100)
                if rand_num < probability:
        ...
        # --- Chat Auto 2: immediately fill in the answer if there's an unanswered message ---
            ...
            if chat_box[0] != "N/A" and chat_box[1] == "N/A":
                probability = random.randint(multiauto1Input.chat_auto2[0], multiauto1Input.chat_auto2[1])
                rand_num = random.randint(0, 100)
                if rand_num < probability:
